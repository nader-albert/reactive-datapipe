...
class HbcTwitterLoader(twitterConfig :Config, transformersHost: String, transformersPort: String) extends StreamLoader {
  ...
  private def setFilters {
    ...
    import na.datapipe.source.engine.twitter.hbc.HbcTwitterLoader._
...
object HbcTwitterLoader {
  def props(twitterConfig :Config, transformersConfig :Config) = {
    ...
    Props(classOf[HbcTwitterLoader], twitterConfig, transformersHost, transformersPort)
  }
